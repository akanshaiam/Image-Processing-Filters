...
int main(void)
{
    ...
    const GLchar* fragment_shader =
            "#version 330 core\n"
					...
					"	for (int i = 0; i < 9; i++) {"
					"		neighbor[i] = texture2D( texture1, TexCoord.xy + (offsets[i] * sharpness) * texOffset );"
					"	}"
					"	FragColor = (neighbor[4] * 9.0) - (neighbor[0] + neighbor[1] + neighbor[2] + neighbor[3] + neighbor[5] + neighbor[6] + neighbor[7] + neighbor[8]);"
                    "}";
